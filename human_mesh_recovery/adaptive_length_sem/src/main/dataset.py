...
class Dataset(object):
    ...
    def __init__(self):
        ...
        if self.config.JOINT_TYPE == 'cocoplus':
            ...
            self.flip_ids_kp2d = tf.constant([5, 4, 3, 2, 1, 0, 11, 10, 9, 8, 7, 6, 12, 13])
    ...
    def get_train(self):
        ...
        dataset = self.create_dataset('train', self._parse,self._random_jitter) #self._random_jitter
        dataset = dataset.shuffle(1000, seed=self.config.SEED, reshuffle_each_iteration=True)
    ...
    def get_val(self):
        ...
        val_dataset = self.create_dataset('val', self._parse, self._convert_and_scale) #self._convert_and_scale
        val_dataset = val_dataset.batch(self.config.BATCH_SIZE, drop_remainder=True)
    ...
    def get_demon(self):
        start = time()
        print('initialize val dataset...')
        demon_dataset = self.create_dataset('test', self._parse,self._convert_and_scale)#self._convert_and_scale
        demon_dataset = demon_dataset.batch(1)
        demon_dataset = demon_dataset.prefetch(8)

        print('Done (t={})\n'.format(time() - start))
        return demon_dataset
    ...
    def _parse(self, example_proto):
        ...
        kp2d=kp2d[:self.config.NUM_KP2D,:]
    ...
    def _parse_new(self, example_proto):
        feature_map = {
            'image_raw_path': tf.io.VarLenFeature(dtype=tf.string),
            'keypoints_2d': tf.io.VarLenFeature(dtype=tf.float32),
            'keypoints_3d': tf.io.VarLenFeature(dtype=tf.float32),
            'has_3d': tf.io.FixedLenFeature([], dtype=tf.int64),
        }
        features = tf.io.parse_single_example(example_proto, feature_map)
        image_data_path = tf.sparse.to_dense(features['image_raw_path'])
        ...
        return image_data_path, kp2d, kp3d, has_3d


    def _convert_and_scale(self, image_data, kp2d, kp3d, has_3d):
        vis = tf.cast(kp2d[:, 2], tf.float32)
        image = tf.image.decode_jpeg(image_data, channels=3)
        # image_data_path = tf.strings.regex_replace(image_data, '/opt/project/tf_data', self.config.DATA_DIR)
        # # print(image_data_path)
        # # image = tf.py_function(_preprocess, [image_data_path], [tf.uint8])
        # raw = tf.io.read_file(image_data_path[0])
    ...
    def _convert_and_scale_demon(self, image_data, kp2d, kp3d, has_3d):
        # vis = tf.cast(kp2d[:, 2], tf.float32)
        image = tf.image.decode_jpeg(image_data, channels=3)
        # image=image_data
    ...
    def _preprocess_demon(self, image_data, kp2d, kp3d, has_3d):
        vis = tf.cast(kp2d[:, 2], tf.float32)
        # image = tf.image.decode_jpeg(image_data, channels=3)
        # helpers.show_image(image.numpy(), kp2d.numpy()[:, :2], vis.numpy())
    ...
    def _random_jitter(self, image_data, kp2d, kp3d, has_3d):
        ...
        #     print(filename)
        #     # filename=bytes.decode(filename[0])
        #     data = sio.loadmat(filename)
        #     print(data)
        #     data = tf.dtypes.cast(data['image'], dtype=tf.uint8)
        #     return data
        #
        # image_data_path = tf.strings.regex_replace(image_data, '/opt/project/tf_data', self.config.DATA_DIR)
        # print(image_data_path)
    ...
    def get_smpl(self):
        ...
        smpl_dataset = smpl_dataset.prefetch(8)
    ...
    def get_test(self):
        ...
        test_dataset = tf.data.TFRecordDataset(tf_records, num_parallel_reads=self.config.NUM_PARALLEL * 2) \
            .map(self._parse, num_parallel_calls=self.config.NUM_PARALLEL * 2) \
            .map(self._convert_and_scale, num_parallel_calls=self.config.NUM_PARALLEL * 2) \
    ...
    #
    #
    # def _convert_and_scale_test(self, image_data, kp2d, kp3d, has_3d):
    #     image = tf.image.decode_jpeg(image_data, channels=3)
    #     # helpers.show_image(image.numpy(), kp2d.numpy()[:, :2], vis.numpy())
    #
    #     # convert to [0, 1].
    ...
    #     image_final = tf.multiply(image_final, 2.0)
    #
    #     return image_final, kp2d, kp3d, has_3d
    ...
    def create_dataset_new(self, ds_type, parse_func, map_func=None, data_dir=None, datasets=None):
        if data_dir is None:
            data_dir = self.config.DATA_DIR
        if datasets is None:
            datasets = self.config.DATASETS

        tf_record_dirs = [join(data_dir, dataset, '*_{}_new.tfrecord'.format(ds_type)) for dataset in datasets]
        tf_records = [tf_record for tf_records in sorted([glob(f) for f in tf_record_dirs]) for tf_record in tf_records]
        print(tf_records)

        dataset = tf.data.Dataset.from_tensor_slices(tf_records)
        # dataset=tf.data.TFRecordDataset(tf_records)tf.data.experimental.AUTOTUNE
        if ds_type != 'test':
        ...
        dataset = dataset.map(parse_func, num_parallel_calls=2)
