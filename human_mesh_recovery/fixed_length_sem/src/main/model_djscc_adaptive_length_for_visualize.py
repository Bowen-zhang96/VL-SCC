...
from main.generator_djscc_adaptive_length_for_visualize import Generator
...
class Model:
    ...
    def test(self, return_kps=True):
        ...
        dataset_train = ExceptionHandlingIterator(tf.data.Dataset.zip((ds_test, ds_smpl)))
        ...
        has_3D_list = []
        count=0
        for image_data,theta in tqdm(dataset_train, total=total, position=0, desc='testing'):
            ...
            bit = self._test_step_for_gradient(image, kp2d, kp3d, has_3d,theta)
            # grads_list.append(grads)
            # bit_list.append(tf.expand_dims(bit,axis=0))
            if count==0:
                bit_list=bit
            else:
                bit_list=tf.concat([bit_list,bit],axis=0)
            has_3D_list.append(tf.expand_dims(has_3d, axis=0))
            count=count+1
            if count%100==0:
                # grads_list_tmp = tf.concat(grads_list, axis=0)
                bit_list_tmp = bit_list
                has_3D_list_tmp = tf.concat(has_3D_list, axis=0)
                sio.savemat('grads_ana7.mat', {'bit_list': bit_list_tmp.numpy(),'has_3D_list': has_3D_list_tmp.numpy()})
        # grads_list_tmp = tf.concat(grads_list, axis=0)
        bit_list_tmp = tf.concat(bit_list, axis=0)
        has_3D_list_tmp = tf.concat(has_3D_list, axis=0)
        sio.savemat('grads_ana7.mat', { 'bit_list': bit_list_tmp.numpy(),
                                       'has_3D_list': has_3D_list_tmp.numpy()})
        # grads_list=tf.concat(grads_list,axis=0)
        # bit_list=tf.concat(bit_list,axis=0)
        # sio.savemat('grads_ana2.mat',{'grads_list':grads_list.numpy(),'bit_list':bit_list.numpy()})
        #     if return_kps:
    ...
    # @tf.function
    def _test_step_for_gradient(self, images, kp2d, kp3d, has3d, theta):
        tf.keras.backend.set_learning_phase(1)
        batch_size = images.shape[0]

        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            # generator_outputs, rate = self.generator(images, training=True)
            gen_tape.watch(images)
            generator_outputs, rates, bits, bits_var, thetas = self.generator(images, training=True)
            # thetas_clean=thetas
            # # only use last computed theta (from iterative feedback loop)
            # loss=[]
            # mask=np.zeros(85)
            ...
            # m=tf.reshape(m,[1,1,85])
            # m=tf.tile(m,[3,1,1])
            # for i in range(0,1,1):
            #     if i==0:
            #         thetas=thetas_clean
            #     else:
            #         thetas=thetas_clean+tf.random.normal(tf.shape(thetas_clean),0,0.1)*m
            #     outputs = []
            #     for i in range(self.config.ITERATIONS):
            #         theta_tmp = thetas[i, :]
            #         outputs.append(self.generator._compute_output(theta_tmp, training=False))
            #
            #     _, kp2d_pred, kp3d_pred, pose_pred, shape_pred, _ = outputs[-1]
            #
            #     vis = tf.expand_dims(kp2d[:, :, 2], -1)
            #     kp2d_loss = v1_loss.absolute_difference(kp2d[:, :, :2], kp2d_pred, weights=vis)
            #     kp2d_loss = kp2d_loss * self.config.GENERATOR_2D_LOSS_WEIGHT
            ...
            #         kp3d_pred = tf.reshape(kp3d_pred, [batch_size, -1])
            #
            #         kp3d_loss = v1_loss.mean_squared_error(kp3d_real, kp3d_pred, weights=has3d_) * 0.5
            #         kp3d_loss = kp3d_loss * self.config.GENERATOR_3D_LOSS_WEIGHT
            #
            #         """Calculating pose and shape loss basically makes no sense
            #             due to missing paired 3d and mosh ground truth data.
            ...
            #     if self.config.USE_3D:
            #         generator_loss = tf.reduce_sum([generator_loss, kp3d_loss])
            #     loss.append(tf.expand_dims(tf.expand_dims(generator_loss,axis=-1),axis=-1))



        # generator_grads = gen_tape.gradient(generator_loss, self.generator.v1)
        # discriminator_grads = disc_tape.gradient(discriminator_loss, self.discriminator.trainable_variables)
        # self.generator_opt.apply_gradients(zip(generator_grads, self.generator.trainable_variables))
        # self.discriminator_opt.apply_gradients(zip(discriminator_grads, self.discriminator.trainable_variables))

        # return tf.concat(loss,axis=-1), bits[-1]
        return bits[-1]
