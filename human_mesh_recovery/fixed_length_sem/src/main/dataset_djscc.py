...
class Dataset(object):
    ...
    def get_train(self):
        ...
        dataset = self.create_dataset_new('train', self._parse_new,self._random_jitter) #self._random_jitter
        # dataset = dataset.shuffle(100, seed=self.config.SEED, reshuffle_each_iteration=True)
    ...
    def get_val(self):
        ...
        val_dataset = self.create_dataset_new('val', self._parse_new, self._convert_and_scale) #self._convert_and_scale
    ...
    def get_test(self):
        ...
        tf_record_dirs = [join(self.config.DATA_DIR, dataset, '*_test_new.tfrecord') for dataset in self.config.DATASETS]
        ...
        test_dataset = tf.data.TFRecordDataset(tf_records, num_parallel_reads=self.config.NUM_PARALLEL * 2) \
            .map(self._parse_new, num_parallel_calls=self.config.NUM_PARALLEL * 2) \
