...
from main.config_fading import Config
from main.dataset import Dataset
from main.discriminator import Discriminator
from main.generator_djscc_adaptive_length_fading_test import Generator
...
class Model:
    ...
    def _build_model(self):
        ...
        self.generator_rate_opt = tf.keras.optimizers.Adam(learning_rate=self.config.GENERATOR_LEARNING_RATE*10.)
        ...
        if not self.config.ENCODER_ONLY:
            checkpoint = tf.train.Checkpoint(generator=self.generator,
                                             ...
                                             generator_rate_opt=self.generator_rate_opt,
    ...
    def _train_step(self, images, kp2d, kp3d, has3d, theta):
        ...
        grade_gen = []
        grade_rate = []
        for grade, var in zip(generator_grads, self.generator.trainable_weights):
            if 'Rate_fun' in var.name:
                var_rate.append(var)
        ...
        self.generator_rate_opt.apply_gradients(zip(grade_rate, var_rate))
        self.generator_opt.apply_gradients(zip(grade_gen, var_gen))
