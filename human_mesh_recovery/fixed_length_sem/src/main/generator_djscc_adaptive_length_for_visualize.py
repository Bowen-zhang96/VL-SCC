...
class Generator(tf.keras.Model):
    ...
    def __init__(self,name='generator'):
        ...
        self.smpl = Smpl()
        # self.v1=tf.Variable(np.zeros([3,1,85]),trainable=True,dtype=tf.float32)
    ...
    def call(self, inputs, **kwargs):
        ...
        # tf.compat.v1.assign(self.v1,thetas)
        ...
        for i in range(self.config.ITERATIONS):
            ...
            bits.append(tf.reduce_sum(m, axis=-1))
            bits_val.append(m_var)
            rates.append(tf.reduce_mean(rate))

            outputs.append(self._compute_output(theta, **kwargs))
            theta_losses.append(tf.reduce_mean(tf.reduce_sum(tf.square(theta - theta), axis=-1) / 85.))

        return outputs, rates, bits, bits_val, thetas
