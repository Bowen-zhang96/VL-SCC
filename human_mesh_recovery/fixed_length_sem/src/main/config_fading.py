...
class Config(object):
    ...
    # LOG_DIR = os.path.join(ROOT_DATA_DIR, 'logs', datetime.now().strftime("%d%m%Y-%H%M%S")) -compression-latent60-lambda360
    LOG_DIR = os.path.join(ROOT_PROJECT_DIR, 'logs', 'lsp-lspet-mpii-smpl-3D-True1-DJSCC2000-snr-20-1024-adaptive-fading')
    # path to specific checkpoint to be restored
    # if LOG_DIR is set to specific training and RESTORE_PATH isone
    ...
    DATA_DIR = os.path.join(ROOT_PROJECT_DIR, 'tf_data')
    ...
    SMPL_DATA_DIR = os.path.join(ROOT_PROJECT_DIR, 'tf_data', 'smpl')
    ...
    DATASETS = ['lsp','lsp_ext','mpii','mpii_3d']
    # DATASETS = ['lsp','lsp_ext','mpii']'lsp','lsp_ext','mpii','mpii_3d'
    ...
    GENERATOR_theta_LOSS_WEIGHT = 60.
    
    GENERATOR_RATE_LOSS_WEIGHT = 60.
    Rate_Lambda =8.    # 9.0 (snr=-15)  11.5 (snr=-20)
    Latent_dim=10
