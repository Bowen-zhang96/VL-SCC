...
from main.config_djscc import Config
from main.dataset import Dataset
from main.discriminator import Discriminator
from main.generator_djscc import Generator
...
class Model:
    ...
    def train(self):
        ...
        print('kp2d_loss_train: %.5f'%(self.kp2d_loss_log.result().numpy()))
        print('kp3d_loss_train: %.5f'%(self.kp3d_loss_log.result().numpy()))
        ...
        for epoch in range(start, self.config.EPOCHS + 1):
            ...
            for image_data, theta in tqdm(dataset_train, total=total, position=0, desc='training'):
                ...
                # generator_outputs, theta_losses = self.generator(images, training=True)
    ...
    def _train_step(self, images, kp2d, kp3d, has3d, theta):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            generator_loss = tf.reduce_sum([kp2d_loss, gen_disc_loss, theta_loss])
            if self.config.USE_3D:
    ...
    def _log_train(self, epoch):
        ...
        if self.config.USE_3D:
            template = 'theta_loss: {}, kp2d_loss: {}, kp3d_loss: {}'
            print(template.format(self.theta_loss_log.result(), self.kp2d_loss_log.result(),  self.kp3d_loss_log.result()))
        else:
            template = 'theta_loss: {}, kp2d_loss: {}'
            print(
                template.format(self.theta_loss_log.result(), self.kp2d_loss_log.result()))
    ...
    def _val_step(self, images, kp2d, kp3d, has3d):
        ...
        result,  theta_losses = self.generator(images, training=False)
    ...
    def _log_val(self, epoch):
        ...
        self.theta_loss_log_test.reset_states()
        if self.config.USE_3D:
    ...
    def test(self, return_kps=True):
        ...
        mpjpe, kps2d_pred, kps2d_real, mpjpe_kp3d, mpjpe_kp3d_aligned, kps3d_pred, kps3d_real = [], [], [], [], [], [], []
        ...
        for image_data in tqdm(ds_test, total=total, position=0, desc='testing'):
            ...
            kp2d_mpjpe, predict_kp2d, real_kp2d, kp3d_mpjpe, kp3d_mpjpe_aligned, predict_kp3d, real_kp3d = self._test_step(
                image, kp2d, kp3d, has_3d, return_kps=return_kps)
            if return_kps:
        ...
        if return_kps:
            ...
            result_dict.update({'kps2d_pred': kps2d_pred, 'kps2d_real': kps2d_real, 'kps3d_pred': kps3d_pred,
    ...
    def _test_step(self, image, kp2d, kp3d, has3d, return_kps=False):
        ...
        result, theta_losses = self.generator(image, training=False)
        # only use last computed theta (from accumulated iterative feedback loop)
        _, kp2d_pred, kp3d_pred, _, _, _ = result[-1]
        vis = kp2d[:, :, 2]
        kp2d_norm = tf.norm(kp2d_pred[:, :self.config.NUM_KP2D, :] - kp2d[:, :, :2], axis=2) * vis
        kp2d_mpjpe_ = tf.reduce_sum(kp2d_norm, axis=1) / tf.reduce_sum(vis, axis=1)
        kp2d_mpjpe = tf.reduce_sum(kp2d_norm) / tf.reduce_sum(vis)
        kp3d_mpjpe = 0.
        ...
        if return_kps:
            return kp2d_mpjpe, kp2d_pred, kp2d, kp3d_mpjpe, kp3d_mpjpe_aligned, kp3d_pred, kp3d,

        return kp2d_mpjpe, None, None, kp3d_mpjpe, kp3d_mpjpe_aligned, None, None
