...
def fading(x, mask):
    ...
    noise_stddev = np.sqrt(10 ** (15 / 10))

    # additive white gaussian noise
    awgn = tf.complex(
        tf.random.normal(tf.shape(x), 0, noise_stddev / np.sqrt(2), dtype=tf.float32),
        tf.random.normal(tf.shape(x), 0, noise_stddev / np.sqrt(2), dtype=tf.float32),
    )

    def grad(upstream):
        return upstream * mask * tf.math.conj(h), upstream * tf.math.conj(x) * tf.math.conj(h)

    return (h * x + awgn) * mask, grad
...
class Channel(tf.keras.Model):
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.channel_type == "awgn":
            ...
            z_mask_com = tf.complex(z_mask[:, :dim_z], 0.0)
            z_in = z_in * z_mask_com

            dim_z = tf.math.real(tf.reduce_sum(tf.stop_gradient(z_mask_com), axis=-1, keepdims=True))
            ...
            z_in = z_in * tf.complex(
                tf.sqrt(tf.cast(dim_z, dtype=tf.float32) / (z_norm + 1e-6)), 0.0
            )
            z_out = fading(z_in, z_mask_com)

            #     # convert back to real
            z_out = tf.concat([tf.math.real(z_out), tf.math.imag(z_out)], axis=-1)
...
class Generator(tf.keras.Model):
    ...
    def __init__(self,name='generator'):
        ...
        self.channel = Channel("fading", -15, name="channel_output")
